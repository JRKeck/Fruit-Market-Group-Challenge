...
var priceChangeInterval = 5000; // In milleseconds 5000 = 5 seconds
var gameLength = 20000;// In milleseconds, 120000 = 2 minutes
var gameTime = 0; // Keep track of how long the game has been running
var gameValid = false;//Set false when the game is over
var storeInventory = [];
...
function StoreItem(name, img) {
	...
	this.priceTrend = true; //track the price trend
}
...
//this will call updateFruitPrice at a set interval determined by priceChangeInterval
function startGame() {
...
//Sell the users fruit at the end of the game
function endGameTally() {
	console.log(storeInventory);
	for (var i = 0; i < storeInventory.length; i++) {
		User.cash += (storeInventory[i].numInCart * storeInventory[i].currentPrice)
	}
	$('.cart .user-cash').text('Total Cash: '+formatNum(User.cash));//update user cash on DOM
	if (User.cash >= 10000){
	$('#endModal .modal-body').append("<p>Congrats you didn't lose any money! You finished the game with "+formatNum(User.cash)+"</p>");
	}
	else {
	$('#endModal .modal-body').append("<p>Oh no, you lost money! You finished the game with "+formatNum(User.cash)+"</p>");
	}
	$('#endModal').modal({
		backdrop: 'static',
		keyboard: false
